...
function isRawLog(content) {
    ...
    if (lines.length >= 6) {
        const noisyLines = lines.filter(l => /^[\s│├─└●·$>|✓✗✔✘]/.test(l.trim()) || // box chars, prompts, bullets
            /^\s*\d+[.:]\s/.test(l) || // numbered lists from tool output
            /^={3,}|^-{3,}|^\*{3,}/.test(l.trim()) // divider lines
        ).length;
        if (noisyLines / lines.length > 0.60)
            return true;
    }
...
const handler = async (event) => {
    ...
    if (event.type !== 'message')
        return;
    if (event.action !== 'preprocessed' && event.action !== 'sent')
        return;
    try {
        ...
        if (isFilteredSession(ctx, content))
            return;
