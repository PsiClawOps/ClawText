...
import { getClawTextProdStateRoot, getClawTextTopicAnchorsDir, } from './runtime-paths.js';
...
function normalizeJsonStringList(value) {
    ...
    if (typeof value === 'string') {
        return uniquePreserveOrder(compactLines(value).map((line) => line.replace(/^[-*]\s*/, '').trim()));
    }
    return [];
}
function parseMarkdownAnchor(topic, markdown) {
    const sections = {
        meta: [],
        current: [],
        decisions: [],
    ...
    return {
        ...
        keyDecisions: uniquePreserveOrder(compactLines(sections.decisions.join('\n')).map((line) => line.replace(/^[-*]\s*/, '').trim())),
        history: uniquePreserveOrder(compactLines(sections.history.join('\n')).map((line) => line.replace(/^[-*]\s*/, '').trim())),
    };
}
function parseJsonAnchor(topic, parsed) {
...
function candidatePaths(workspacePath, topic) {
...
export function loadTopicAnchor(workspacePath, topic) {
    for (const filePath of candidatePaths(workspacePath, topic)) {
        ...
        try {
            ...
            const parsed = filePath.endsWith('.json')
                ? parseJsonAnchor(topic, JSON.parse(raw))
...
export function serializeTopicAnchor(anchor) {
    const parts = [`# ${anchor.topic}`, ''];
    parts.push('## Meta');
    ...
    if (anchor.keyDecisions.length === 0) {
        ...
        for (const line of anchor.keyDecisions)
            parts.push(`- ${line}`);
    }
    parts.push('');
    parts.push('## History');
    if (anchor.history.length === 0) {
        ...
        for (const line of anchor.history)
            parts.push(`- ${line}`);
    }
    parts.push('');
    return parts.join('\n');
}
export function saveTopicAnchor(workspacePath, anchor) {
...
function extractDecisionCandidates(recentContent) {
    ...
    return uniquePreserveOrder(recentContent.filter((line) => decisionSignals.some((pattern) => pattern.test(line))));
}
...
export function syncTopicAnchor(workspacePath, params) {
    ...
    if (!anchor.meta.created)
        anchor.meta.created = now;
    anchor.meta.updated = now;
    anchor.meta.session = params.sessionKey;
    if (params.channelId && params.channelId !== 'unknown')
        anchor.meta.channel = params.channelId;
    if (params.channelName)
        anchor.meta.channelName = params.channelName;
    if (params.lastSender)
        anchor.meta.lastSender = params.lastSender;
    anchor.meta.lastTrigger = params.trigger;
    const nextStatus = summarizeCurrentStatus(params.recentContent, anchor.currentStatus);
    if (nextStatus)
        anchor.currentStatus = nextStatus;
...
export function formatTopicAnchorForSlot(anchor, options) {
    ...
    const truncateLines = (content, maxLines) => {
