 ...
 */
function resolveNameFromIdentityFile(workspacePath: string): string | null {
  const identityPath = join(workspacePath, 'IDENTITY.md');
  if (!existsSync(identityPath)) return null;
  try {
    const lines = readFileSync(identityPath, 'utf-8').split('\n');
 ...
 */
function mapRoleToTier(roleText: string): 'council' | 'director' | 'worker' {
...
export function resolveAgentIdentity(workspacePath: string, config?: ClawTextMultiAgentConfig): AgentIdentity {
  ...
  const workspacePatterns: Array<{ pattern: string; defaultRole: 'council' | 'director' | 'worker' }> = [
